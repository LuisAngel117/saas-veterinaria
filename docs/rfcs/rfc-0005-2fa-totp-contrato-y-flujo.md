...
- Login devuelve 409 Problem Details con code=AUTH_2FA_REQUIRED y challengeId (TTL corto).
...
## 5) Impacto
- Impacto tecnico:
  - Agregar campos a user_account: totp_enabled, totp_secret (base32 cifrado), totp_verified_at.
  - Agregar tabla auth_2fa_challenge (id, user_id, expires_at, used_at, created_at).
  - Endpoints nuevos bajo /api/v1/auth/2fa/*.
- Impacto UX:
  - UI debe manejar challengeId y pedir codigo TOTP.
- Impacto seguridad:
  - Secreto TOTP debe almacenarse cifrado/hashed (definir en implementacion).
...
1) Definir contrato de endpoints y codigos de error (en este RFC).
2) Acordar estrategia de auditoria para AUTH_2FA_RESET antes de SPR-B004:
   - Opcion A: crear audit_event minimo en SPR-B003 (subset),
   - Opcion B: mover SPR-B004 antes de SPR-B003,
   - Opcion C: permitir reset solo despues de SPR-B004 (SPR-B003 parcial).
3) Actualizar docs/sprints/SPR-B003.md con contrato final (si se aprueba).
